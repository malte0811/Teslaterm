...
export class ScopeIPC {
    ...
    public startControlledDraw(source?: object) {
        this.processIPC.sendToWindow(IPCConstantsToRenderer.scope.startControlled, source);
    }

    public drawLine(x1: number, y1: number, x2: number, y2: number, color: number, source?: object) {
        this.processIPC.sendToWindow(
            IPCConstantsToRenderer.scope.drawLine, source, new ScopeLine(x1, y1, x2, y2, color),
        );
    }

    public drawText(x: number, y: number, color: number, size: number, str: string, center: boolean, source?: object) {
        this.processIPC.sendToWindow(
            IPCConstantsToRenderer.scope.drawString, source, new ScopeText(x, y, color, size, str, center),
        );
    }
    ...
    public configure(
        ...
        this.processIPC.sendToAll(IPCConstantsToRenderer.scope.configure, config);
        this.configs.set(traceId, config);
    }

    public updateMediaInfo() {
        this.processIPC.sendToAll(IPCConstantsToRenderer.scope.redrawMedia,
    ...
    public sendConfig(source: object) {
        for (const cfg of this.configs.values()) {
            this.processIPC.sendToWindow(IPCConstantsToRenderer.scope.configure, source, cfg);
        }
    }

    private tick() {
        if (Object.keys(this.tickSummary).length > 0) {
            this.processIPC.sendToAll(IPCConstantsToRenderer.scope.addValues, new ScopeValues(this.tickSummary));
            this.tickSummary = [];
        }
    }
}
